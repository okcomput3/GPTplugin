    ...
    for(i = 0; i < view.len; i++) {
        if(view.data[i] == c) {
            if(arr_index < arr_s-2) {
                String_View new_view = {.data = cur, .len = view.data + i - cur};
                arr[arr_index++] = new_view;
                cur = view.data + i + 1;
            } else {
                String_View new_view = {.data = view.data + i+1, .len = view.len - i-1};
                arr[arr_index++] = new_view;
                return arr_index;
            }
        }
...
Token generate_word(String_View *view, char *contents) {
    ...
    return (Token){.type = Type_None, .index = 0, .size = 0};
}
...
size_t generate_tokens(char *line, size_t line_s, Token *token_arr, size_t *token_arr_capacity) {
    ...
    while(view.len > 0) {
        ...
        token_arr = (Token*)realloc(token_arr, sizeof(Token)*(*token_arr_capacity)*2);
